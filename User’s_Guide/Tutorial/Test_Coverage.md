...
Для запуска тестов используйте команду pytest. Он найдет и запустит все написанные вами
тестовые функции.
...
Если какие-либо тесты не пройдены, pytest покажет возникшую ошибку. Вы можете запустить 
pytest -v, чтобы получить список каждой тестовой функции, а не точки.
Чтобы измерить покрытие кода ваших тестов, используйте команду покрытия для запуска pytest,
а не запускайте его напрямую.
$ coverage run -m pytest

Вы можете просмотреть простой отчет о покрытии в терминале:
$ coverage report
...
Отчет в формате HTML позволяет увидеть, какие строки были покрыты в каждом файле:
$ coverage html

Это создает файлы в каталоге htmlcov. Откройте htmlcov/index.html в браузере, чтобы
просмотреть отчет.
